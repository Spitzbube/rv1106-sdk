...
export RK_PROJECT_FILE_OEM_SCRIPT=$RK_PROJECT_OUTPUT/S6appinit
...
function choose_target_board()
{
	...
	do
		...
		echo "----------------------------------------------------------------"
		itemstr=${item##*/}
		echo -e "${C_GREEN}$cnt. $itemstr${C_NORMAL}"
		cnt=$(( cnt + 1 ))
		f0=${item#BoardConfig*-}
		boot_medium=${f0%%-*}
...
EOF
	...
	mkdir -p $RK_PROJECT_OUTPUT_IMAGE
	$RK_PROJECT_PATH_PC_TOOLS/mk-fitimage.sh $RK_PROJECT_PATH_RAMDISK/boot4recovery.its $RK_PROJECT_PATH_RAMDISK/${ramdisk_file} $kernel_image $kernel_dtb_file $RK_PROJECT_PATH_RAMDISK/resource.img $RK_PROJECT_OUTPUT_IMAGE/recovery.img $RK_ARCH
...
EOF
	...
	__COPY_FILES $SDK_SYSDRV_DIR/rootfs_overlay $RK_PROJECT_PACKAGE_ROOTFS_DIR
