...
network_init()
{
	...
	sleep 2
    ethip=`ifconfig | grep -Eo 'inet addr:[0-9.]+' | grep -v '127.0.0' | awk -F':' '{print $2}' | awk -F'.' '{print $1"."$2"."$3}' | sort | uniq`
    sleep 1
    ifconfig eth0 $ethip.168
    sleep 2s
    echo "add:`route add default gw $ethip.1`"
...
post_chk()
{
	...
	if [ `ls -l /dev/media* | wc -l` -le 2 ]; then
		ln -s -f /etc/rkipc.ini $default_rkipc_ini
	fi
