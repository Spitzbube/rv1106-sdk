...
void dwc3_set_mode(struct dwc3 *dwc, u32 mode)
{
	...
	if (mode == DWC3_GCTL_PRTCAP_HOST){
		if(gpio_is_valid(otg_gpio)){
			//printk("rpdzkj: Switch to HOST mode, pull up otg_gpio!\n");
			gpio_set_value(otg_gpio, 1);
		}
	}

	if (mode == DWC3_GCTL_PRTCAP_DEVICE){
		if(gpio_is_valid(otg_gpio)){
			//printk("rpdzkj: Switch to DEVICE mode, pull down otg_gpio!\n");
...
static int dwc3_probe(struct platform_device *pdev)
{
	...
	otg_gpio = of_get_named_gpio(node, "otg_gpio", 0);
	if (gpio_is_valid(otg_gpio)){
		ret = gpio_request(otg_gpio, "otg_gpio");
		if(ret)
			dev_err(dev, "otg_gpio request error!!\n");
		gpio_direction_output(otg_gpio, 0);
	}else{
		dev_err(dev, "faild to get otg_gpio, maybe not need or config error\n");
	}
