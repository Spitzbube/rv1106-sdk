...
static struct gpio_desc *power_gpio;
...
{
	int version[2];
	int hactive, vactive, hfp, hsync, hbp, vfp, vsync, vbp, htotal, vtotal;
	int lanes;
	...
	power_gpio = devm_gpiod_get_optional(&client->dev, "power", GPIOD_ASIS);
        if (IS_ERR(power_gpio)) {
                dev_err(&client->dev, "lt8912 failed to request power GPIO\n");
        }else{
	...
	if(gpio_is_valid(reset_gpio)){
		...
		msleep(100);
		gpiod_direction_output(reset_gpio, reset_flag);
		msleep(100);
